...
class Calender_skill():
    ...
    def add_event(self, shiwi:AI)->bool:
        shiwi.say("What is the name of the event")
        try:
            event_name = shiwi.listen()
            shiwi.say("When is this event?")
            event_begin = shiwi.listen()
            event_isodate = dateparser.parse(event_begin).strftime("%Y-%m-%d %H:%M:%S")
            shiwi.say("What is the event description?")
            event_description = shiwi.listen()
            ...
            return False
    def remove_event(self, shiwi:AI)->bool:
        shiwi.say("What is the name of the event you want to remove?")
        try:
            event_name = shiwi.listen()
            try:
                self.calendar.remove_event(event_name=event_name)
                shiwi.say("Event removed successfully")
                self.calendar.save()
                return True
            except:
                shiwi.say("Sorry I could not find the event",event_name)
                return False
        except:
            print("opps there was an error")
            return False

    def list_events(self, period, shiwi:AI)->bool:
